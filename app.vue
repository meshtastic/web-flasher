<template>
  <div>
    ...
    <button @click="monitorSerial" class="inline border border-meshtastic focus:ring-4 focus:outline-none font-medium rounded-lg text-xs px-4 py-1 text-center me-2 mb-2 text-meshtastic hover:text-black hover:bg-white hover:border-transparent hover:shadow transition duration-300 ease-in-out">
      Open Serial Monitor <CommandLineIcon class="h-4 w-4 inline mb-1" />
    </button>
    <button @click="goToGitHub" class="inline border border-meshtastic focus:ring-4 focus:outline-none font-medium rounded-lg text-xs px-4 py-1 text-center me-2 mb-2 text-meshtastic hover:text-black hover:bg-white hover:border-transparent hover:shadow transition duration-300 ease-in-out">
      Contribute on GitHub
      <!-- Directly embed the SVG and apply dynamic styling -->
...
const goToGitHub = () => {
  window.open('https://github.com/meshtastic/web-flasher', '_blank');
};
