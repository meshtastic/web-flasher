<template>
  <div v-if="releaseNotes && !firmwareStore.hasSeenReleaseNotes" class="mb-4">
    <h1>
...
<script lang="ts" setup>
  ...
  watch(() => firmwareStore.selectedFirmware?.release_notes, async () => {
    if ((firmwareStore.selectedFirmware?.release_notes || '').length === 0) {
      releaseNotes.value = undefined;
    }
  
    const output = await marked(firmwareStore.selectedFirmware?.release_notes || '');
