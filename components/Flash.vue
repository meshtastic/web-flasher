<template>
    <div>
        <button data-modal-target="flash-modal" data-modal-toggle="flash-modal"
            class="block text-black meshtastic-bg hover:bg-gray-200 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 disabled:bg-gray-500 text-center"
        ...
        <div id="flash-modal" tabindex="-1" aria-hidden="true"
            ...
            <TargetsNrf52 v-if="deviceStore.$state.selectedTarget?.architecture == 'nrf52840' || deviceStore.$state.selectedTarget?.architecture == 'rp2040'" />
            <TargetsEsp32 v-if="deviceStore.$state.selectedTarget?.architecture?.includes('esp32')" />
        </div>
    </div>
</template>
